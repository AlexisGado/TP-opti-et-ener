...
def create_thermal_plant_lp( pb,name):
    ...
    for thermal_plant in pb.thermal_plants :
        L = int(thermal_plant.minimum_online_duration/pb.time_step_duration)
        
        for t in range(L,pb.number_of_time_steps):
            
            constraint_name_dur = "dur_"+ str(t)+"_"+thermal_plant.name            
            lp+=pulp.lpSum([switch_vars[tp][thermal_plant] for tp in range(t-L+1,t+1)])<=status_vars[t][thermal_plant], constraint_name_dur
